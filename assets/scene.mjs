...
export function sceneFrame(progress, width, height, still = false, screen = aperture) {
  ...
  const cx = sx + sw / 2;
  const cy = sy + sh / 2;
  const finalScale = Math.max(width / sw, height / sh) * 1.08;
  ...
  const inset = mobile ? 16 : clamp(width * .035, 28, 64);
  const final = { x: inset, y: mobile ? 86 : 104, width: width - inset * 2, height: Math.max(200, height - (mobile ? 140 : 158)) };
  const takeover = still ? (p >= .5 ? 1 : 0) : smooth(.76, 1, p);
  const shell = {
...
export function mountScene() {
  ...
  function paint() {
    ...
    progress = clamp(-journey.getBoundingClientRect().top / range);
    const frame = sceneFrame(progress, width, height, quiet());
    world.style.width = `${frame.imageWidth}px`;
    world.style.height = `${frame.imageHeight}px`;
    ...
    shell.style.left = `${frame.shell.x}px`;
    shell.style.top = `${frame.shell.y}px`;
    shell.style.width = `${frame.shell.width}px`;
    shell.style.height = `${frame.shell.height}px`;
    shell.style.borderRadius = `${4 + 10 * smooth(.7, 1, progress)}px`;
    desktop.style.width = `${frame.final.width}px`;
    desktop.style.height = `${frame.final.height}px`;
  ...
  function refresh() { root.classList.toggle('is-still', quiet()); update(); }
